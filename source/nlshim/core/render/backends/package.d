module nlshim.core.render.backends;
...
version (InDoesRender) {
    version (UseQueueBackend) {
        public import nlshim.core.render.backends.queue;
    } else {
        public import nlshim.core.render.backends.opengl;
        version (RenderBackendDirectX12) {
            public import nlshim.core.render.backends.directx12;
        }
    }
}

version (UseQueueBackend) {
    static import nlshim.core.render.backends.queue;
} else {
    static import nlshim.core.render.backends.opengl;
}

version (UseQueueBackend) {
    import nlshim.core.render.backends.queue;
    alias RenderBackend = nlshim.core.render.backends.queue.RenderingBackend!(BackendEnum.OpenGL);
} else {
    template RenderingBackend(BackendEnum backendType) {
        static if (backendType == BackendEnum.OpenGL) {
            alias RenderingBackend = nlshim.core.render.backends.opengl.RenderingBackend!(backendType);
        } else static if (backendType == BackendEnum.DirectX12) {
            alias RenderingBackend = nlshim.core.render.backends.directx12.RenderingBackend!(backendType);
        } else {
